...
public class ProgramManagerReviewController {
	...
	public ResponseEntity<List<String>> decide(@PathVariable Long id,
			@Valid @RequestBody ProgramManagerDecisionRequest req) {

		log.info("PM decision received. projectId={}, decision={}", id, req.getDecision());

		ResearchProjectResponse result = service.decide(id, req.getDecision(), req.getReason(), req.getAmount());

		if ("APPROVED".equalsIgnoreCase(req.getDecision())) {
			return ResponseEntity
					.ok(List.of("Project approved successfully", "Grant issued", "Project ID: " + result.getProjectId(),
							"Amount: " + req.getAmount(), "Status: " + result.getStatus()));
		}

		return ResponseEntity.ok(List.of("Project rejected successfully", "Reason: " + result.getReason(),
				"Project ID: " + result.getProjectId(), "Status: " + result.getStatus()));
	}
}